...
var setStickersPriceValuesHtml = function ($container, marketHashName, imagesDiv) {
  ...
  if (!$container.parent().find('.sticker-price-info').length) {
    $container.after(`
                    <div class="sticker-price-info">
                    <p class="sticker-wear-info-value">${stickersData[marketHashName]}</p>
                </div>
               `);

  }
}
var setStickersPriceValues = function (data) {
  ...
  for (let i = 0; i < imagesDiv.length; i++) {
    ...
    const market = 'sihanalyst';

    const $container = imagesDiv.eq(i);
    if (stickersData[marketHashName]) {
      setStickersPriceValuesHtml($container, marketHashName, imagesDiv);
    } else {
      chrome.runtime.sendMessage(SIHID, {
        ...
        if (res.success && res.items[marketHashName] && res.items[marketHashName][market]) {
          const price = res.items[marketHashName][market].price;
          if (price) {
            const priceFormat = SIH?.Currency.getPriceFromCurrency(price, currencyId);
            stickersData[marketHashName] = priceFormat.text.replace('pуб.', '₽');
            setStickersPriceValuesHtml($container, marketHashName, imagesDiv);
          }
        }
      })
