...
async function sellBtn() {
  ...
  const fromCurrencyCode = +(typeof (g_rgWalletInfo) != 'undefined' && g_rgWalletInfo['wallet_currency'] ? g_rgWalletInfo['wallet_currency'] : 1);

  const mpriceFormated = (SIH?.global?.Currency?.getPriceFromCurrency(price, currencyId, fromCurrencyCode)).text;
